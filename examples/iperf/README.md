...
Based on the Morse Micro MM-IoT-SDK iperf example, extended with web configuration, NAPT, DNS forwarding, and multi-language support.
...
- [MM-IoT-SDK](https://github.com/MorseMicro/MM-IoT-SDK) (Morse Micro framework)
- Environment variable **`MMIOT_ROOT`** set to the path of the framework directory inside the MM-IoT-SDK (the one that contains `framework/`, `morselib/`, etc.)

## Building

From the **MM-IoT-SDK** repo root (or the path where `MMIOT_ROOT` points to the framework):

```bash
cd examples/iperf
export MMIOT_ROOT=/path/to/mm-iot-esp32   # if not already set
...
Buffer size is chosen at **build time** via `sdkconfig.defaults`:
...
To use Small or Medium, copy the matching file over `sdkconfig.defaults`, then build:
